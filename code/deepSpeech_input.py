...
def _parse_function(example_proto):
    ...
    #return context_parsed["seq_len"], tf.cast(context_parsed["labels"].values,tf.int32), sequence_parsed["feats"]
    return tf.cast(context_parsed["seq_len"],tf.int32), tf.serialize_sparse(tf.cast(context_parsed["labels"],tf.int32)), sequence_parsed["feats"]
...
def inputs(eval_data, data_dir, batch_size, shuffle=False):
    ...
    dataset = dataset.padded_batch(batch_size,padded_shapes=([],[None],[None,13]))
    dataset = dataset.filter(lambda t,y,s: tf.equal(tf.shape(t)[0], batch_size))
